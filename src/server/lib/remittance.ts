...
// USDC has 6 decimal places. 1 USDC = 1_000_000 minor units.
export const USDC_DECIMALS = 6;
export const USDC_MINOR_UNIT = 1_000_000n;
...
export function phpToUsdcMinor(phpAmount: number): string {
  ...
  const minor = Math.round(usdc * 1_000_000);
...
export function buildSep7PayUri(params: {
  ...
  const amount = (Number(BigInt(params.amountUsdc)) / 1_000_000).toFixed(7);
