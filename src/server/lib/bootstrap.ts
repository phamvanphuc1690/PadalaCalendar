...
export function ensureBootstrap(): void {
  ...
  // Public hackathon previews use the in-memory remittance demo store. Do not
  // start database-backed sweepers/pollers when no database was configured.
  if (process.env.DEMO_MODE === 'true' || !process.env.DRIZZLE_DATABASE_URL) return;
  handles.push({ stop: startExpirySweeper() });
  handles.push({ stop: startWithdrawalPoller() });
  if (env.BASE_ENABLED && env.HUB_EVM_ADDRESS) {
