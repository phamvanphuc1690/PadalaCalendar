 ...
 *
 * The bundled non-public demo can sign a challenge locally for fixture flows.
 * Mainnet and production paths fail closed and require the merchant's external
 * wallet/HSM to sign the challenge.
 *
...
/**
 * Sign a SEP-10 challenge XDR for the non-public demo fixture only.
 *
 * A real deployment must have the merchant sign via Freighter/HSM and return
 * the signed XDR to the server, which would then POST it to the anchor.
 */
export async function signSep10ChallengeLocally(
  anchorDomain: string,
  account: string,
): Promise<string> {
  if (env.DEMO_MODE !== true || env.STELLAR_NETWORK === 'public') {
    throw new AppError(
      'CONFLICT',
      'SEP-10 requires an external wallet signature outside non-public demo mode',
      409,
    );
  }
