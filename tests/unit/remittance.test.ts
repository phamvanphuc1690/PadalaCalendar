...
describe('usdcToString', () => {
  it('converts 1 USDC correctly', () => {
    expect(usdcToString(1_000_000n)).toBe('1.00');
  });

  it('converts 86.21 USDC correctly', () => {
    expect(usdcToString(86_210_000n)).toBe('86.21');
  });
...
describe('phpToUsdc', () => {
  it('converts ₱5800 to 100 USDC', () => {
    expect(phpToUsdc(5800)).toBe(100_000_000n);
  });

  it('converts ₱58 to 1 USDC', () => {
    expect(phpToUsdc(58)).toBe(1_000_000n);
  });
});
...
describe('usdcToPhp', () => {
  it('converts 1 USDC to ₱58', () => {
    expect(usdcToPhp(1_000_000n)).toBe('₱58.00');
  });
...
describe('stringToUsdc', () => {
  it('converts decimal string to bigint', () => {
    expect(stringToUsdc('86.21')).toBe(86_210_000n);
  });

  it('converts whole number string', () => {
    expect(stringToUsdc('100')).toBe(100_000_000n);
  });

  it('converts 1.00 correctly', () => {
    expect(stringToUsdc('1.00')).toBe(1_000_000n);
  });
});
