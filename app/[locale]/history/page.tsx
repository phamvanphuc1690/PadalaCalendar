...
import { getRecentEvents, getSchedules } from '@/server/service/remittance.service';
import { usdcToString } from '@/server/lib/usdc';

export const dynamic = 'force-dynamic';

export default async function HistoryPage() {
  const allRemittances = await getSchedules();
  const recentNotifications = await getRecentEvents(10);
        ...
        <main className="flex-1 p-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm mb-8">
            <table className="w-full">
              ...
              <tbody>
                {allRemittances.map(({ schedule: rem, recipient: r }) => (
